  ...
  await Account.findOne({ email: email, google_id: null, facebook_id: null })
    .then(async (result) => {
      if (!result) {
        ...
        const dehashPassword = await comparePassword(password, result.password);
  ...
  await Account.findOne({ email: email })
    .then(async (result) => {
      if (result && !result.google_id && !result.facebook_id) {
        ...
        // const verificationCode = Math.floor(100000 + Math.random() * 900000);
        // let link = "https://127.0.0.1:8000/account/verify/" + token.code;
  ...
  try {
    ...
    if (!token) {
      return res.json({
        success: false,
        message:
          "We were unable to find a valid token. Your token might have expired.",
      });
    }
